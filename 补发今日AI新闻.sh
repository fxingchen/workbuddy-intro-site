...
if [ ! -f "feishu_config.json" ]; then
    ...
    echo ""
    echo "请先创建配置文件:"
    echo "1. 复制模板: cp feishu_config_template.json feishu_config.json"
...
# 检查AI新闻报告
if [ ! -f ".codebuddy/automations/ai/2026-03-27_ai_news.md" ]; then
...
# 询问是否继续
read -p "确认补发今日AI新闻到飞书? (y/n): " confirm
if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
    echo "操作已取消"
    exit 0
fi
...
echo ""
echo "⏳ 正在补发今日AI新闻..."
python3 src/ai_news_automation_with_feishu.py
